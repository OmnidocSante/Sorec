...
public class RdvService {
    ...
    public List<RdvRecord> getAllAppointments() {
        return rdvRepo.findAll().stream().map(rdv -> new RdvRecord(rdv.getDate(), rdv.getJockey().getUser().getNom(), rdv.getJockey().getUser().getPrénom(), rdv.getMedecin().getUser().getNom(), rdv.getMedecin().getUser().getPrénom(), rdv.getStatusRDV())).toList();
    }
    ...
    public List<RdvRecord> getDoctorAppointments(String jwt) {
        ...
        return rdvRepo.findRdvsByMedecin(medecin).stream().map(rdv -> new RdvRecord(rdv.getDate(), rdv.getJockey().getUser().getNom(), rdv.getJockey().getUser().getPrénom(), rdv.getMedecin().getUser().getNom(), rdv.getMedecin().getUser().getPrénom(), rdv.getStatusRDV())).toList();
