...
export default function ExamenCardio() {
  ...
  const fetchData = async () => {
    const response = await instance.get(`/api/jockey/${id}/examen-cardio`);
  ...
  const handleToggle = (conditionId, value) => {
    setConditions((prev) => ({
      ...
      parametresExamenCardioVasculaires:
        prev.parametresExamenCardioVasculaires.map((cond) =>
          cond.id === conditionId
            ? { ...cond, hasCondition: value.toString() }
            : cond
        ),
    }));
    console.log(conditions);
  ...
  const handleRemarksChange = (conditionId, value) => {
    setConditions((prev) => ({
      ...
      parametresExamenCardioVasculaires:
        prev.parametresExamenCardioVasculaires.map((cond) =>
          cond.id === conditionId ? { ...cond, observations: value } : cond
        ),
    }));
  };
