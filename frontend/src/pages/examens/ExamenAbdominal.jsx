...
export default function ExamenAbdominal() {
  ...
  const handleSave = async (data) => {
    ...
    try {
      ...
      if (!hiddenFields.has("oropharynx"))
        payload.oropharynx = data.oropharynx === "" ? null : data.oropharynx;
      if (!hiddenFields.has("foie"))
        payload.foie = data.foie === "" ? null : data.foie;
      if (!hiddenFields.has("rate"))
        payload.rate = data.rate === "" ? null : data.rate;
  ...
  return (
    ...
    >
      ...
      {!hasVisibleData && !loading && (
        ...
        >
          Aucune donnée d'examen abdominal enregistré ou visible pour ce
          dossier.
        </motion.div>
      )}
    </motion.form>
