...
export default function ExamenAuditif() {
  ...
  const examenAuditifSchema = z.object({
    id: z.number(),
    reflexePupillaire: z.string().min(1, "Réflexe pupillaire est requis"),
    reflexesOstéoTendineux: z.string().min(1, "Réflexes ostéo-tendineux sont requis"),
    coordination: z.string().min(1, "Coordination est requise"),
    equilibre: z.string().min(1, "Équilibre est requis"),
    sensibilite: z.string().min(1, "Sensibilité est requise"),
    motricite: z.string().min(1, "Motricité est requise"),
    tonicite: z.string().min(1, "Tonicité est requise"),
  ...
  const fetchData = async () => {
    try {
      const response = await instance.get(
        `/api/jockey/${id}/examen-auditif`
      );
      reset({
        id: response.data.id,
        reflexePupillaire: response.data.reflexePupillaire ?? "",
        reflexesOstéoTendineux:
          response.data.reflexesOstéoTendineux ?? "",
