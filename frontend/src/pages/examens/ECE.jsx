...
export default function ECR() {
  ...
  const handleSave = async (data) => {
    ...
    try {
      ...
      await instance.put(`/api/jockey/${id}/electrocardiogramme-effort`, payload);
  ...
  const fetchItem = async (dossierid) => {
    fetchData(`/api/jockey/${id}/electrocardiogramme-effort/historique/${dossierid}`);
    setIsHistory(true);
    setIsEditMode(false);
    setShowHistorique(false);
  };
    ...
    >
      ...
      {hasVisibleData && (
       ...
       {!hasVisibleData && !loading && (
           ...
           >
             Aucune donnée d'électrocardiogramme enregistrée ou visible pour ce dossier.
           </motion.div>
       )}
    </motion.form>
  );
